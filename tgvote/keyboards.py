...
def change_score_perfomance(perf_id,keyb_message):
    keyb=InlineKeyboardMarkup()
    keyb.row(InlineKeyboardButton(keyb_message,callback_data=f"perfomance_{perf_id}"))
    return keyb

def score_keyboard(id_perf: int, excluded=[]) -> InlineKeyboardMarkup:
    keyb = InlineKeyboardMarkup()
    for i in range(1, 11):
